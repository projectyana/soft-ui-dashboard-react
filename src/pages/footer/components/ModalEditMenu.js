...
const ModalEditMenu = ({ fetchParent, modalConfig, setModalConfig }) => {
  ...
  const formSubmitHandler = (values, { setSubmitting }) => {
    ...
    FooterApi.update(modalConfig.data.id, finalValue)
      .then(res => {
  ...
  const getInitialData = () => {
    ...
    Axios.all([
      FooterApi.getSingle(modalConfig.data.id),
      FooterApi.getPageUnlinked(),
      FooterApi.getBlogUnlinked(),
      GalleryApi.getGalleryCategories(),
      FileUploadApi.getCategories(),
    ])
      .then((Axios.spread((resSingle, resPage, resBlog, resGalleryCat, resFileCat) => {
